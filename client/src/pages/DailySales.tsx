...
export default function DailySales() {
  ...
  useEffect(() => {
    // pegamos os valores vindos do backend
    const valuesFromBackend: Record<string, number> = {};
    sales.forEach((s) => {
      valuesFromBackend[s.id] = Number(s.salesValue);
    });

    // MERGE: preserva edições locais (prev) e preenche com backend quando não existir
    setEditingValues((prev) => ({ ...valuesFromBackend, ...prev }));
  }, [sales]);
  ...
  const handleSave = (id: string, newValue: number) => {
    ...
    // prepara payload para o recálculo (envia todo o histórico atualizado)
    const allSalesUpdated = sales.map((s) => ({
      ...s,
      totalSold: s.id === id ? newValue : Number(s.salesValue),
      // assegura chaves esperadas pelo backend se necessário
      minGoal: s.minGoal,
      maxGoal: s.maxGoal,
    }));
    ...
    // chama o mutate que reconstrói as metas no backend (ou faz patch conforme sua implementação)
    recalculateGoals.mutate({
      saleId: id,
      newSalesValue: newValue.toString(),
      allSales: allSalesUpdated,
      totalMinGoal,
      totalMaxGoal,
    });
  };
  ...
  const handleGenerateDays = () => {
    ...
    // calculateDailyGoals suporta salesHistory opcional (aqui passa vazio)
