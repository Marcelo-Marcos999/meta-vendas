...
export default function Dashboard() {
  ...
  const totalSales = sales.reduce((sum, s) => sum + Number(s.sales_value), 0);
  
  // Usar valores da configuração (metas totais do período) em vez da soma das metas diárias
  ...
  const daysWithSales = sales.filter((s) => Number(s.sales_value) > 0).length;
  const avgDailySales = daysWithSales > 0 ? totalSales / daysWithSales : 0;

  const chartData = sales.map((s) => ({
    date: format(parseISO(s.date), "dd/MM"),
    vendas: Number(s.sales_value),
    metaMin: Number(s.min_goal),
    metaMax: Number(s.max_goal),
  }));
      ...
      <div className="space-y-6 animate-fade-in">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <Card className="stat-card">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-foreground">
                {formatCurrency(config?.max_goal || 0)}
              </div>
          ...
          <Card className="stat-card">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-foreground">
                {formatCurrency(config?.min_goal || 0)}
              </div>
